...
#ERROR: "API returned an empty response (e.g. location in ocean or not in U.S.). NA returned for elevation"
#EPQS uses API so this is expected, AWS elevations are found for all retained occurrences

write.csv(df_fl, file = "O_digyna_df_fl_elev_2025.csv")

#df_fl <- read.csv("O_digyna_df_fl_elev_2025.csv") #After running initially, you can bypass get_elevation
  #download and pull in .csv directly
#==============================================================================#
...
#2243 observations retained

write.csv(df_cl, file = "O_digyna_df_fl_elev_clean_2025.csv")

#df_cl <- read.csv("O_digyna_df_fl_elev_clean_2025.csv") #Can bypass clean_data step and just pull in .csv
...
#alpine: 117 obs; tundra: 110 obs; boreal: 106 obs; all: 333 obs (now))
{ 
...
{
  df_first <- bind_rows(df_first_alpine, df_first_arc_tun, df_first_bor_arc)
}
...
df_first <- na.omit(df_first)
df_first_arc_tun <- na.omit (df_first_arc_tun)
df_first_bor_arc <- na.omit (df_first_bor_arc)
df_first_alpine <- na.omit(df_first_alpine)

#Regression tests
#ALL_subset
regression <- lm(formula = first_day ~ Year, data = df_first)
summary(regression)
regression <- lm(formula = first_day ~ Mean_temperature, data = df_first)
summary(regression)
regression <- lm(formula = first_day ~ June_temperature, data = df_first)
summary(regression)

#ARCTIC_TUNDRA
regression <- lm(formula = first_day ~ Year, data = df_first_arc_tun)
summary(regression)
regression <- lm(formula = first_day ~ Mean_temperature, data = df_first_arc_tun)
summary(regression)
regression <- lm(formula = first_day ~ June_temperature, data = df_first_arc_tun)
summary(regression)

#BOREAL_ARCTIC
regression <- lm(formula = first_day ~ Year, data = df_first_bor_arc)
summary(regression)
regression <- lm(formula = first_day ~ Mean_temperature, data = df_first_bor_arc)
summary(regression)
regression <- lm(formula = first_day ~ June_temperature, data = df_first_bor_arc)
summary(regression)

#ALPINE
regression <- lm(formula = first_day ~ Year, data = df_first_alpine)
summary(regression)
regression <- lm(formula = first_day ~ Mean_temperature, data = df_first_alpine)
summary(regression)
regression <- lm(formula = first_day ~ June_temperature, data = df_first_alpine)
summary(regression)

#AIC for Linear regression models calculated along with SAR model output (lm)
...
{
  custom_colors <- c("All" = "darkviolet", "Arctic tundra" = "#0AB1D6", "Boreal Arctic" = "#418A00",  "Alpine" = "#C81A00")
